...
__global__ void partial_least_squares_cg_kernel(int first_user,
                                        int factors, int user_count, int item_count,
    ...
    for (int u = first_user + blockIdx.x; u < first_user + user_count; u += gridDim.x) {
        float * x = &X[u * factors];

        // handle 0-sized rows
        if (indptr[u - first_user] == indptr[u - first_user + 1]) {
            x[threadIdx.x] = 0;
            continue;
        }
        ...
        for (int it = 0; it < cg_steps; ++it) {
            ...
            for (int index = indptr[u - first_user]; index < indptr[u - first_user + 1]; ++index) {
                const float * Yi = &Y[indices[index] * factors];
                float confidence = data[index];
...
void CudaPartialLeastSquaresSolver::least_squares_init(
    ...
    if (Y.cols != YtY.cols) throw invalid_argument("Columns of Y don't match number of factors");
...
void CudaPartialLeastSquaresSolver::least_squares(
    ...
    partial_least_squares_cg_kernel<<<block_count, thread_count, shared_memory_size>>>(
        first_user,
...
__global__
void partials_calculate_loss_kernel(int first_user, int factors, int user_count, int item_count,
...
float CudaPartialLeastSquaresSolver::calculate_loss(
                                            int first_user,
                                            int user_count,
                                            const CudaCSRMatrix & Cui,
    ...
    partials_calculate_loss_kernel<<<1024, factors, sizeof(float) * factors>>>(
        first_user,
...
CudaPartialLeastSquaresSolver::~CudaPartialLeastSquaresSolver() {
